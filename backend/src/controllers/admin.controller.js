...
export const listSupportRequestsHandler = asyncHandler(async (req, res) => {
  const data = await supportRequestsService.listAll(req.query.status);
  res.json({ data });
});

export const updateSupportRequestHandler = asyncHandler(async (req, res) => {
  const id = parseInt(req.params.requestId, 10);
  if (isNaN(id)) throw Object.assign(new Error('VALIDATION'), { message: 'Invalid support request ID' });
  const data = await supportRequestsService.updateStatus(id, req.body.status);
  res.json({ data });
});
