...
export function getPlanDetails(value) {
  ...
  if (!key) return null;
  return PLAN_LIMITS[key] ?? { label: key, memberLimit: null, monthlyRoleplayMinutes: null };
}
...
export function hasReachedMemberLimit(schoolOrPlan, memberCount) {
  const details = typeof schoolOrPlan === 'object'
    ? getEffectivePlanDetails(schoolOrPlan)
